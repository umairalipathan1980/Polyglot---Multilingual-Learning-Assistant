...
SUPPORTED_LANGUAGES = {
    ...
    "jpn": {"name": "Japanese", "flag": "🇯🇵", "native_name": "日本語"},
    "kor": {"name": "Korean", "flag": "🇰🇷", "native_name": "한국어"},
    "zho": {"name": "Chinese", "flag": "🇨🇳", "native_name": "中文"},
    "rus": {"name": "Russian", "flag": "🇷🇺", "native_name": "Русский"},
    "ara": {"name": "Arabic", "flag": "🇸🇦", "native_name": "العربية"},
    "por": {"name": "Portuguese", "flag": "🇵🇹", "native_name": "Português"},
    "hin": {"name": "Hindi", "flag": "🇮🇳", "native_name": "हिन्दी"},
    "swe": {"name": "Swedish", "flag": "🇸🇪", "native_name": "Svenska"},
    "nld": {"name": "Dutch", "flag": "🇳🇱", "native_name": "Nederlands"},
    "tur": {"name": "Turkish", "flag": "🇹🇷", "native_name": "Türkçe"},
}
...
# # Configure page
# st.set_page_config(
#     page_title="Maikki - Finnish Language Tutor",
#     page_icon="🇫🇮",
#     layout="wide",
#     initial_sidebar_state="expanded"
# )
...
# # Sidebar
# with st.sidebar:
#     st.markdown("<h1 class='sidebar-title'>🇫🇮 Maikki</h1>", unsafe_allow_html=True)
    
#     # Language level selection with detailed info
#     st.markdown("### Select your Finnish level")
...
#         index=level_options.index(st.session_state.selected_level),
#         key="level_selector",
#         help="Choose your current Finnish language proficiency level"
#     )
...
#         # Add a system message about the level change
#         level_message = f"Your Finnish level has been changed to {selected_level}. All content will now be adapted to this level."
#         st.session_state.messages.append({"role": "assistant", "content": level_message})
#         st.session_state.chat_history.append({
#             "role": "assistant", 
#             "content": level_message, 
...
#     # File upload (now accepts any file type)
#     st.markdown("### Upload File")
#     uploaded_file = st.file_uploader("Upload a file with Finnish content", type=None)
...
#                 # Add a system message to inform the user that the file was uploaded
#                 system_msg = f"{level_badge} File '{uploaded_file.name}' has been uploaded. You can now ask questions about it, request translations of text in the file, or ask for exercises based on it that are adapted to your {level_code} level."
#                 st.session_state.messages.append({"role": "assistant", "content": system_msg})
#                 st.session_state.chat_history.append({
#                     "role": "assistant", 
#                     "content": system_msg, 
...
#         # Encode to download
#         b64 = base64.b64encode(markdown_text.encode()).decode()
#         file_name = f"maikki_chat_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
#         href = f'<a href="data:text/markdown;base64,{b64}" download="{file_name}">Click to download chat history (Markdown)</a>'
#         st.markdown(href, unsafe_allow_html=True)
...
#             to_badge = format_level_badge(to_code)
#             date = change["timestamp"].split()[0]
#             level_history_md += f"{from_badge} → {to_badge} on {date}<br>"
...
# # Use markdown for title with HTML instead of st.title
# st.markdown(f"""<div class="main-title">🇫🇮 Maikki - Your Finnish Language Tutor {level_badge}</div>""", unsafe_allow_html=True)

# st.markdown(f"""
# <div class="welcome-card">
#     <div class="welcome-header">
#         <div class="welcome-icon">🇫🇮</div>
#         <h2>Welcome to Maikki</h2>
#     </div>
#     <div style="text-align: left; font-size: 15px; margin-top: 20px; line-height: 1.6;">
#         <p>I'm Maikki, your adaptive Finnish language tutor. I can help you with:</p>
#         <ul style="list-style-position: inside; text-align: left; display: inline-block;">
#             <li><strong>Translations</strong> - between Finnish and English (use format: "T: text")</li>
#             <li><strong>Grammar explanations</strong> - including verb types, cases, and consonant gradation</li>
#             <li><strong>Personalized exercises</strong> - reading, writing, vocabulary, and quizzes</li>
#             <li><strong>Assignment help</strong> - upload your assignment in any file format</li>
#             <li><strong>Content-based learning</strong> - upload files to learn from text, images, or documents</li>
#         </ul>
#         <div style="margin-top: 15px; padding: 15px; border-radius: 8px; border-left: 4px solid {get_level_color(level_code)};" class="level-bg-{level_code}">
#             <p style="margin: 0; font-weight: bold;">
#                 {level_badge} You're currently learning at the {st.session_state.selected_level} level
#             </p>
#             <p style="margin-top: 8px; font-size: 0.9rem;">
#                 {level_descriptions.get(st.session_state.selected_level, "")}
#             </p>
...
#             with st.chat_message("assistant"):
#                 # Insert level badge for system messages about level changes
#                 if "Finnish level has been changed" in message['content'] or "has been uploaded" in message['content']:
#                     # Keep the level badge if it's already there
#                     if not message['content'].startswith('<span class="level-badge'):
#                         # Find which level this message was for
#                         if i > 0 and "Finnish level has been changed to" in message['content']:
#                             # Extract the level from the message
#                             for level_option in level_options:
#                                 if level_option in message['content']:
#                                     level = level_option.split()[0]
...
#         now = datetime.now().hour
#         if now < 12:
#             greeting = "Hyvää huomenta! (Good morning!) 🌞"
#         elif now < 18:
#             greeting = "Hyvää päivää! (Good afternoon!) 🌤️"
#         else:
#             greeting = "Hyvää iltaa! (Good evening!) 🌙"
        
#         # Format greeting with level badge
#         level_badge = format_level_badge(level_code)
        
#         intro_message = f"{greeting} I'm Maikki, your Finnish language tutor. {level_badge} You've selected the {st.session_state.selected_level} level. I'll adapt all my responses, exercises, and vocabulary to this level. How can I help you today?"
#         st.session_state.messages.append({"role": "assistant", "content": intro_message})
#         st.session_state.chat_history.append({
#             "role": "assistant", 
#             "content": intro_message, 
